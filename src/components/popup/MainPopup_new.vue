<template>
  <modal :name="'popupModal'+popIndex" :ref="'popupModal'+popIndex"
        :width=imgWidth
        :height=imgHeight
        :draggable="false"
        :resizable="false"
    ...
    <div id="layer-popup">
      <div class="popup-header">
        <h2>{{popTitle}}</h2>
        <button type="button" class="popup-close" @click="popupTodayClose($event, popIndex)">
          <img src="../../assets/img/popup_close.png" alt="팝업닫기" title="팝업닫기">
        </button>
      </div>
      <div class="popup-cnt">
        <div class="img-area" v-html="popImg"></div>
        <div class="popup--option">
          ...
          <button type="button" class="popup-close" @click="popupTodayClose($event, popIndex)" style="margin-left:10px;"> [닫기] </button>
        </div>
      </div>
    </div>
  </modal>
...
export default {
  ...
  mounted() {
    const popupOpenDate = new Date(this.popList.popStartDate.replace(/,/g, '-'))
    const popupEndDate = new Date(this.popList.popEndDate.replace(/,/g, '-'))
    const toDate = new Date()   
    
    if (popupOpenDate.getTime() <= toDate.getTime() && toDate.getTime() <= popupEndDate.getTime()) {      
      const popupCloseDay = Cookies.get(this.boardId)
      const toDay = CommonUtil.getFormatDate(new Date, '')      
      if (!popupCloseDay) {
        this.showPopup(this.popIndex)
      } else {
        if (popupCloseDay === toDay) {
          this.closePopup(this.popIndex)
        } else {
          Cookies.remove(this.boardId)
          this.showPopup(this.popIndex)
        }
      }
    }
  ...
  methods: {
    showPopup(idx) {       
      this.$modal.show('popupModal'+idx)
    },

    closePopup(idx) {
      this.$modal.hide('popupModal'+idx)
    },

    popupTodayClose(event, idx) {
      this.$nextTick(function () {
        if (event) event.preventDefault()
        if (this.isCheckedTodayClose) {
          Cookies.set(this.boardId, CommonUtil.getFormatDate(new Date, ''))
        }
        this.closePopup(idx)
      })
    }
...
#layer-popup .popup-header h2{color:#ffffff;font-weight: 500;font-size: 17px;padding-left:8px;width:92%;}
...
#layer-popup .popup-cnt ::v-deep img{max-width: 100%;}
