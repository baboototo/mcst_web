<template>
  <modal name="popupModal1" ref="popupModal1"
         :width="600"
         :height="700"
         :draggable="false"
         :resizable="false"
    ...
    <div id="layer-popup">
      ...
      <div class="popup-cnt">
        <div class="img-area">
          <img src="../../assets/popupImg/kmunhwaPopup.png" alt="메인 팝업 이미지" title="메인 팝업 이미지">
        </div>
...
export default {
  ...
  mounted() {
    ...
    if (popupOpenDate.getTime() <= toDate.getTime() && toDate.getTime() <= popupEndDate.getTime()) {
      const popupCloseDay = Cookies.get('kmunhwaPopupCloseDay')
      ...
      if (!popupCloseDay) {
        ...
        if (popupCloseDay === toDay) {
          ...
          Cookies.remove('kmunhwaPopupCloseDay')
          this.showPopup()
        }
      }
    }
    ...
    popupTodayClose(event) {
      this.$nextTick(function () {
        ...
        if (this.isCheckedTodayClose) {
          Cookies.set('kmunhwaPopupCloseDay', CommonUtil.getFormatDate(new Date, ''))
        }
        this.closePopup()
      })
    }
...
.vm--overlay[data-modal="popupModal1"] {
  width: 0px;
  height: 0px;
